...
class NotificationController {
    ...
    func scheduleHourlyNotifications(hoursInADay: Int, minutesInAHour: Int, secondsInAMinute: Int) {
        ...
        for i in 0..<hoursInADay {
            ...
            let triggerComponents = TimeUtils.getCurrentTime(adjustedTimeHours: i, adjustedTimeMinutes: 0, adjustedTimeSeconds: 0, minutesInAHour: minutesInAHour, secondsInAMinute: secondsInAMinute)
            let triggeredTime = TimeUtils.formatTime(hours: triggerComponents.hour!, minutes: triggerComponents.minute!, seconds: triggerComponents.second!)
